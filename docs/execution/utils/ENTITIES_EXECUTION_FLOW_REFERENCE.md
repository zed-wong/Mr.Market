...
| `server/src/common/entities/market-making/strategy-order-intent.entity.ts` | `StrategyOrderIntentEntity` | Durable intent queue/state table for controller/orchestrator-created strategy intents, including execution category and metadata for AMM swaps. | `executor-orchestrator.service.ts`, `strategy-intent-store.service.ts`, `strategy-intent-worker.service.ts`, reconciliation | 6, 7 |
...
- Keep strategy entity rows aligned with `docs/plans/2026-03-04-dynamic-strategy-architecture-transition-plan.md` after each schema/versioning update.
