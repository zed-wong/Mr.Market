...
- Dual-account volume hedging is maker-ack-driven, but no longer "blind immediate submit": after maker ACK, `StrategyIntentExecutionService` now waits for the maker order to be open/unfilled, validates that the maker exclusively owns the top level at its price, optionally applies up to `1s` of random jitter, revalidates, and only then submits the taker IOC.
- Dual-account volume now treats self-match safety as a hard execution guard: if the maker loses best-price ownership before taker dispatch, or if taker fill is not paired by the maker fill delta, the runtime marks the cycle anomalous and cancels the remaining maker leg best-effort instead of silently treating the taker leg as success.
