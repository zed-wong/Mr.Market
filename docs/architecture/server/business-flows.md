...
- Dual-account volume hedging is now fill-driven instead of placement-driven: the strategy publishes only the maker leg on tick, listens to private order/trade streams for both accounts, and emits taker IOC intents only from observed maker fill deltas.
- Dual-account volume no longer exposes or consumes legacy maker-delay / inline-settlement config: the hedge path is driven entirely by observed maker fill deltas, and admin/config surfaces now match that runtime model.
