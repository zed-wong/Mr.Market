...
- `efficientDualAccountVolume` reuses the pooled executor on one exchange:pair but sequences maker placement on one account and taker IOC execution on a second account, with restart recovery cancelling dangling maker orders instead of replaying taker legs.
- `efficientDualAccountVolume.tradedQuoteVolume` is the strategy progress metric for actual taker-leg filled quote only; gross maker+taker turnover stays a separate reporting concept.
...
- Removed dual-account volume controller aliases are cleaned from strategy definitions by migration and should not be treated as supported runtime types.
...
- Efficient dual-account hedging is maker-ack-driven, but no longer "blind immediate submit": after maker ACK, `StrategyIntentExecutionService` now waits for the maker order to be open/unfilled, validates that the maker exclusively owns the top level at its price, optionally applies up to `1s` of random jitter, revalidates, and only then submits the taker IOC.
- Efficient dual-account now treats self-match safety as a hard execution guard: if the maker loses best-price ownership before taker dispatch, or if taker fill is not paired by the maker fill delta, the runtime marks the cycle anomalous and cancels the remaining maker leg best-effort instead of silently treating the taker leg as success.
- Efficient dual-account cycles now read live balances for the selected maker/taker pair before publishing: oversized configured amounts are reduced to the currently affordable quantity, the runtime can retry the opposite side when the preferred side has no tradable balance path, and zero-sized or raw below-min post-balance quotes are dropped before CCXT precision helpers run.
- When both normal efficient dual-account sides are blocked by inventory, the runtime emits a one-leg IOC rebalance order from the strategy layer; rebalance candidates that still fail raw exchange minimums or precision quantization are skipped without failing the whole tick, and successful rebalance fills are accounted like any other trade without advancing dual-account cycle counters or triggering any taker hedge path.
- Efficient dual-account settlement does not treat every maker/taker fill difference as repair-worthy: exact matches and configured small mismatches complete normally, below-exchange-minimum dust is carried forward, and only material paired-fill mismatches enter repair with a persisted repair context.
- Efficient dual-account repair mode first checks whether paired execution can resume. If the stored repair context is already below exchange minimums, repair is cleared as carried dust; otherwise repair tries a targeted IOC against the exact mismatched account/side/quantity before falling back to the generic one-leg rebalance path.
- Efficient dual-account repair states that still cannot build a min-valid resume or rebalance action use the classified no-progress guard; deterministic empty planner states stop the controller, while transient cache/balance blockers wait through the bounded guard.
- Efficient dual-account runtime treats repeated empty decisions as bounded no-progress: the same readiness blocker for three ticks emits `STOP_CONTROLLER` with `dual_account_no_progress_*`, and published-cycle counters advance only for real maker `CREATE_LIMIT_ORDER` actions, never stop/cancel/rebalance intents.
