...
- Efficient dual-account repair mode first checks whether paired execution can resume; deterministic below-min repair states emit a `STOP_CONTROLLER` intent instead of looping on empty repair ticks, while transient cache/balance blockers use the bounded no-progress guard.
