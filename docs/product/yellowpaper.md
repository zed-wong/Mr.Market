...
| `StrategyTemplate` | Complete reusable parameter preset under a strategy definition. Defines named parameter sets that operators/users select when creating an order. | Admin-manageable |
| `strategySnapshot` | Order-level frozen runtime strategy config resolved from definition + template + runtime fields. Runtime uses this, not live definitions/templates. | Immutable after creation |
