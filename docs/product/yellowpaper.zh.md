...
Order Balance 是运行时额度检查和 reservation 的 read model。余额绑定到 ledger order scope，而非仅绑定用户或交易所账户。下单、成交、撤单、fee 和 PnL 都必须通过 Ledger Entry 表达，并更新订单级余额视图。

订单身份词汇表：

| 名称 | 含义 | 主要用途 |
|------|------|----------|
| `userOrderId` | 产品和用户可见的订单身份。 | 订单详情、UI/API 读取、PnL、奖励、提现和报表按这个 id 聚合。 |
| `ledgerOrderId` | 余额桶身份。 | Ledger Entry、OrderBalance、reservation、成交结算、手续费结算、提现和对账余额检查都按 `ledgerOrderId + asset` 归属。 |
| `accountLabel` | `userOrderId` 下的执行账户或策略腿，例如 `default`、`maker`、`taker`。它本身不是 order id。 | 交易所路由、dual-account scope 解析、tracked order 和对账报表。 |
| `clientOrderId` | 下单前生成并提交给 venue 的客户端订单 id。 | 下单幂等和 fill routing；提交到交易所的安全格式必须通过 `ExchangeOrderMapping` 映射回内部订单。 |
| `exchangeOrderId` | 交易所或链上执行 venue 返回的订单 id。 | fetch、cancel、open-order reconciliation 和成交匹配。它绝不能作为余额 scope。 |

单账户订单通常使用 `userOrderId === ledgerOrderId`，并且 `accountLabel = 'default'`。Dual-account 订单共享一个 `userOrderId`，同时使用不同 ledger scope，例如 `ledgerOrderId = <userOrderId>:maker` 和 `ledgerOrderId = <userOrderId>:taker`；这些 scope 只能通过 centralized ledger-order-scope helper 解析或构造。

当前 ledger、balance、tracked-order 和 exchange-order-mapping 存储里名为 `orderId` 的数据库列，语义上存的是 `ledgerOrderId`。新的 service contract 在 `userOrderId` 和 `ledgerOrderId` 都可能存在时，不得继续传裸 `orderId`。

### 2.4 余额字段

每个 ledger order scope 按资产维护以下字段：
...
3. `reserve_lock` 必须按 `ledgerOrderId + asset` 串行化，并且不得让 `available` 小于 0；
...
| `OrderBalance` | 资金层 / Balance 模块 | 某个 ledger order scope、某个资产的当前余额视图 |
| `Reservation` | 资金层规则，可选独立模块 | 策略下单前对订单余额的临时锁定；MVP 可以由 ledger entry + intent / tracked order 引用表达，详见 2.6 |

每条 `LedgerEntry` 必须包含：`ledgerOrderId`、`userOrderId`、`accountLabel`、`asset`、`amount`、`type`、`idempotencyKey`、`refType`、`refId` 和 `createdAt`。当前存储可以把 `ledgerOrderId` 列命名为 `orderId`；语义字段仍然是 `ledgerOrderId`。Ledger Entry 创建后不可修改；任何修正必须通过新的 reversal entry 表达。
...
`OrderBalance` 更新必须按 `ledgerOrderId + asset` 串行化。实现可以使用数据库 row lock、乐观锁版本号或等价机制。无论采用哪种方式，必须保证：
...
| `StrategyTemplate` | 某个策略定义下的可复用参数预设。定义运营人员 / 用户创建订单时可选择并覆盖的具名默认参数集。 | 可由后台管理 |
| `strategySnapshot` | 订单级冻结运行时策略配置，由 definition + template + order overrides + runtime fields 解析生成。运行时使用它，而不是实时读取 definitions/templates。 | 创建后不可变 |
...
| 策略实例绑定 | 每个策略实例绑定一个 `userOrderId` 和一个执行账户标签；余额影响仍然通过解析后的 `ledgerOrderId` 结算 |

约束：

1. 每个交易所订单和成交都必须携带 `userOrderId`、`ledgerOrderId` 和 `accountLabel`，从而归因回发起它的 Mr.Market 订单；
...
| 同一 `ledgerOrderId + asset` | reservation / balance mutation 串行执行 |
