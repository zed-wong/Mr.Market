...
Order Balance 是运行时额度检查和 reservation 的 read model。余额绑定到 ledger order scope，而非仅绑定用户或交易所账户。`userOrderId` 是用户可见订单身份，用于订单详情、PnL 和报表聚合。`ledgerOrderId` 是 Ledger Entry 和 OrderBalance 使用的余额桶身份。单账户订单的 `userOrderId` 和 `ledgerOrderId` 通常相同；dual-account 订单可以共享一个 `userOrderId`，同时使用 maker / taker 等不同 ledger scope。下单、成交、撤单、fee 和 PnL 都必须通过 Ledger Entry 表达，并更新订单级余额视图。
