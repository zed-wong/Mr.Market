...
- Wire the remaining `web3-interface` wallet/account funding surfaces to real web3 APIs: `/app/wallet` now loads `/web3/balances` with available/in-market-making sections and persisted funding activity, `/app/deposit` loads `/web3/deposit/instructions` and submits `/web3/deposit/verify`, `/app/withdraw` validates available ledger balances before calling `/web3/withdraw` and polling status, and `/app/account` now exposes wallet/network, Reown network switching, language, theme, and disconnect controls.
- Add authenticated `/web3/withdraw` and `/web3/withdraw/:withdrawalId`: withdrawals now validate ledger-derived wallet availability, persist idempotent withdrawal status rows, debit the web3 wallet ledger exactly once, submit ERC-20 transfers through the configured Web3 signer when available, and expose tx hash or blocked/failed evidence with owner-only status lookup.
