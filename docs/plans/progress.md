...
- Complete Phase 4 AMM swap settlement foundation: route `EXECUTE_AMM_SWAP` through `EvmDexConnector` without generic submit retry; reserve token-in and funding-operator gas before nonce preallocation; broadcast swaps once with explicit nonce/gas overrides; add `swap_settle` and `gas_debit` ledger types, AMM/gas reservation release helpers, TokenRegistry-based `AmmSwapSettlementService`, child approve/wrap/unwrap execution planning, `AmmVolumeStrategyController`, EVM execution reconciliation, and wallet-vs-ledger reconciliation; verify with focused AMM/EVM/ledger/strategy specs plus server build.
- Complete Phase 5 CLMM LP lifecycle foundation: extend the EVM DEX adapter contract with LP mint/increase/decrease/collect/read methods; add `OrderLpPosition`, LP position service, pool state tracker, LP settlement service, structural LP reconciliation runner, `LiquidityProvisionStrategyController`, CLMM intent types, and LP ledger types (`lp_add_settle`, `lp_remove_settle`, `lp_fee_credit`) that move funds through `externalLocked`; route LP intents through `EvmDexConnector` with nonce/gas sponsorship and async `EvmExecution` tracking; verify LP open/rebalance/collect/close-focused specs plus server build.
- Complete Phase 6 Hyperliquid CLOB hardening: initialize Hyperliquid connector accounts from valid `TradingAccount` rows with purpose `clob_trading` while preserving the existing API-key path for other CLOBs; add explicit Hyperliquid lifecycle, 128-bit `clientOrderId`, fee normalization, restart recovery, and reconciliation mismatch blocking coverage; verify with focused Hyperliquid/CLOB specs plus server build.
- Complete Phase 7 reconciliation and operational safety wiring: coordinate CLOB order polling, EVM execution reconciliation, wallet-vs-ledger reconciliation, and LP position reconciliation through `ReconciliationService`; carry order identity in EVM/LP runner reports; emit manual-review audit events and pause affected ledger/wallet balance reservations on mismatches; verify cross-family blocking coverage plus server build.
- Complete Phase 8 admin UI updates for AMM direct market making: expose sanitized DEX setup metadata, support first-class `ammVolume` creation with connector/chain/token/fee/sizing/slippage/deadline/cadence and DEX/gas account selectors, validate AMM setup before start, and add admin order status visibility for EVM executions, LP positions, gas/fee/swap/LP ledger facts, and reconciliation reservation blocks; verify with focused server/admin-interface specs plus server build, admin-interface check, and admin-interface build.
