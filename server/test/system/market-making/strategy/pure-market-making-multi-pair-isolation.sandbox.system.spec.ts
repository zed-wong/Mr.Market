...
const log = createSystemTestLogger('pure-mm-multi-pair-isolation');

if (skipReason) {
  logSystemSkip('pure market making multi-pair isolation suite', skipReason);
}

const describeSandbox = skipReason ? describe.skip : describe;

describeSandbox(
  'Pure market making multi-pair isolation parity (system)',
  () => {
    ...
    it('keeps executor sessions isolated across two pairs on the same exchange', async () => {
      ...
      const firstIntents = await helper.listStrategyIntents(
        first.order.orderId,
      );
