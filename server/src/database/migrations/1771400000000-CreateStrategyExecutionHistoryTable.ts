  ...
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    const marketMakingHistoryTables: Array<{ name: string }> =
      await queryRunner.query(
        `SELECT name FROM sqlite_master WHERE type='table' AND name='market_making_history'`,
      );
    const arbitrageHistoryTables: Array<{ name: string }> =
      await queryRunner.query(
        `SELECT name FROM sqlite_master WHERE type='table' AND name='arbitrage_history'`,
      );
    const indicatorStrategyHistoryTables: Array<{ name: string }> =
      await queryRunner.query(
    ...
    if (marketMakingHistoryTables.length > 0) {
      await queryRunner.query(
    ...
    if (indicatorStrategyHistoryTables.length > 0) {
      await queryRunner.query(
