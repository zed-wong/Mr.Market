...
export class AdminAnalyticsController {
  ...
  @Get('orders/:orderId')
  @ApiOperation({
    summary: 'Read per-order PNL, inventory, and risk analytics',
  })
  @ApiQuery({
    name: 'exchange',
    required: false,
    description: 'Exchange override used to resolve the live order-book mid.',
  })
  @ApiQuery({
    name: 'pair',
    required: false,
  ...
  async getOrderAnalytics(
    ...
    return await this.analyticsService.getOrderAnalytics(orderId, {
      exchange,
      pair,
      startAt,
      endAt,
  ...
  @Get('direct-market-making/dashboard')
  @ApiOperation({
    summary: 'Read Direct Market Making cost and revenue dashboard analytics',
  })
  @ApiQuery({
    name: 'scope',
    required: false,
    enum: ['admin', 'pair', 'order'],
    description: 'Dashboard scope. Defaults to admin.',
  })
  @ApiQuery({
    name: 'orderId',
    required: false,
  ...
  async getDirectMarketMakingDashboard(
    @Query('scope') scope?: unknown,
    @Query('orderId') orderId?: unknown,
    @Query('exchange') exchange?: unknown,
    @Query('pair') pair?: unknown,
    ...
    return await this.analyticsService.getDirectMarketMakingDashboard({
      scope,
      orderId,
      exchange,
      pair,
