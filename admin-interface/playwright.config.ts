...
export default defineConfig({
  ...
  webServer: [
    {
      command:
        "bun run --cwd ../server build && rm -f ../server/data/admin-e2e.db ../server/data/admin-e2e.db-* && PORT=3000 DATABASE_PATH=data/admin-e2e.db ADMIN_PASSWORD=test-admin-password JWT_SECRET=test-admin-jwt-secret-with-enough-length-for-local-validation ENCRYPTION_PRIVATE_KEY=MDEyMzQ1Njc4OWFiY2RlZjAxMjM0NTY3ODlhYmNkZWY= CORS_ORIGIN=http://localhost:4174 ADMIN_PASSKEY_RP_ID=localhost ADMIN_PASSKEY_ORIGIN=http://localhost:4174 ADMIN_PASSKEY_RP_NAME='Mr.Market Admin' bun run --cwd ../server start:prod",
