<script lang="ts">
  ...
  async function UpdateMarketMakingPair(id: string, enable: boolean) {
    ...
    const token = localStorage.getItem("admin-access-token");
    if (!token) return;
    await updateMarketMakingPair(id, { enable }, token);
    setTimeout(() => {
      invalidate("admin:settings").finally(() => {
        isUpdating = "";
      });
    ...
    <details class="dropdown dropdown-end" bind:open={addDialog}>
      ...
      >
        <div class="p-2 flex flex-col gap-4">
          ...
          <div class="flex flex-col gap-2">
            <span class="label"> {$_("base_symbol")} </span>
            <input
              type="text"
              class="input input-bordered focus:outline-none"
          ...
          <div class="flex flex-col gap-2">
            <span class="label"> {$_("base_icon_url")} </span>
            {#if !AddNewBaseIconUrl && validate(AddNewBaseAssetId)}
              ...
              >
                <span
                  class={clsx(
                    isBaseIconFetching && "loading loading-spinner loading-sm",
                  )}
                >
                  {$_("fetch")}
                </span>
