...
import {
  ...
  userAssets,
  userSpotOrders,
  userSpotOrdersLoaded,
  userStrategyOrdersLoaded,
...
export const mixinShare = (
  ...
  if (!appId) {
    console.error("Bot id is not initialized");
    return;
  }
...
export const mixinInvoice = async ({
  recipient,
  asset_id,
  amount,
  memo = "",
...
export const mixinUserMe = async (token: string) => {
  ...
  const result = await axios.get(MIXIN_API_BASE_URL + "/me", config);
...
export const mixinTopAssets = async () => {
  const result = await axios.get(MIXIN_API_BASE_URL + `/network/assets/top`);
...
export const mixinAuthWrapper = async (pkce: boolean = false) => {
  ...
  authorize(
    { clientId, scope: OAUTH_SCOPE, pkce },
    {
