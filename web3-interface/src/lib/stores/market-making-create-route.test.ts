...
const routeSource = () =>
  readFileSync(
    fileURLToPath(new URL('../../routes/app/market-making/create/+page.svelte', import.meta.url)),
    'utf8'
  );

describe('/app/market-making/create legacy route', () => {
  it('redirects to the order-first creation route without rendering removed UX', () => {
    const source = routeSource();

    expect(source).toContain("goto('/app/market-making/order/new'");
