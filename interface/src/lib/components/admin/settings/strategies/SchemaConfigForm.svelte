...
<div class="flex flex-col gap-3">
    ...
    <div class="grid gap-3">
      {#each Object.entries(properties) as [key, prop]}
        ...
        {#if fieldType === "boolean"}
          <div class="flex items-center gap-3 bg-base-200/40 rounded-xl p-4">
            <label class="flex items-center gap-2 cursor-pointer flex-1" for="field-{key}">
              <input
                id="field-{key}"
                type="checkbox"
                ...
                checked={Boolean(val)}
                disabled={readOnly}
                on:change={(e) => setFieldValue(key, e.currentTarget.checked)}
              />
              <span class="text-sm text-base-content">
                {key}
                {#if isReq}<span class="text-error ml-0.5">*</span>{/if}
              </span>
            </label>
          </div>
          ...
          <div class="bg-base-200/40 rounded-xl p-4">
            <label class="flex flex-col gap-1.5" for="field-{key}">
              <span class="text-xs font-semibold text-base-content/50 tracking-wider">
                {key}{#if isReq}<span class="text-error ml-0.5">*</span>{/if}
              </span>
              <input
                id="field-{key}"
                type="number"
                ...
                value={val}
                disabled={readOnly}
                on:input={(e) => setFieldValue(key, parseFloat(e.currentTarget.value) || 0)}
              />
              {#if prop.description}
                <span class="text-xs text-base-content/40">{prop.description}</span>
              {/if}
            </label>
          </div>
        {:else if prop.enum && prop.enum.length > 0}
          <div class="bg-base-200/40 rounded-xl p-4">
            <label class="flex flex-col gap-1.5" for="field-{key}">
              ...
              <select
                id="field-{key}"
                class="select select-bordered w-full h-10 min-h-[40px] bg-base-100 text-base-content text-sm focus:outline-none focus:border-primary border-base-300"
                value={val || ""}
                disabled={readOnly}
                on:change={(e) => setFieldValue(key, e.currentTarget.value)}
              >
          ...
          <div class="bg-base-200/40 rounded-xl p-4">
            <label class="flex flex-col gap-1.5" for="field-{key}">
              ...
              <input
                ...
                class="input input-bordered w-full h-10 min-h-[40px] bg-base-100 text-base-content text-sm font-mono focus:outline-none focus:border-primary border-base-300"
                value={Array.isArray(val) ? JSON.stringify(val) : ""}
                disabled={readOnly}
                on:input={(e) => {
                  try {
          ...
          <div class="bg-base-200/40 rounded-xl p-4">
            <label class="flex flex-col gap-1.5" for="field-{key}">
              ...
              <input
                ...
                value={val}
                disabled={readOnly}
                on:input={(e) => setFieldValue(key, e.currentTarget.value)}
              />
              {#if prop.description}
                <span class="text-xs text-base-content/40">{prop.description}</span>
              {/if}
  ...
  {#if !readOnly}
    <div class="flex items-center gap-2 pt-1">
      ...
      />
      <label for="show-raw-json" class="text-xs text-base-content/50 cursor-pointer select-none">
