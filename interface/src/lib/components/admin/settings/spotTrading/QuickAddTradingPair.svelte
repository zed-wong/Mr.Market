<script lang="ts">
  ...
  async function fetchExchangeMarkets(exchangeId: string, token: string) {
    if (marketsByExchange[exchangeId] || loadingExchangeIds.includes(exchangeId)) {
      return;
    }
    ...
    try {
      ...
      loadingExchangeIds = loadingExchangeIds.filter((id) => id !== exchangeId);
    }
  }
  ...
  function getChainIds(assets: any[]): string[] {
    return assets
      ...
      .filter((chainId): chainId is string => typeof chainId === "string");
  }
  ...
  async function submitPair(market: any, baseAsset: any, quoteAsset: any) {
    ...
    const pair: SpotTradingPair = {
      id: getUuid(),
      symbol: market.display_symbol || market.symbol,
      exchange_id: market.exchange_id,
      ccxt_id: market.id || market.symbol,
      base_asset_id: baseAsset.asset_id,
      quote_asset_id: quoteAsset.asset_id,
      amount_significant_figures:
        amountPrecision !== undefined ? String(amountPrecision) : "",
      price_significant_figures:
        pricePrecision !== undefined ? String(pricePrecision) : "",
      buy_decimal_digits:
        pricePrecision !== undefined ? String(pricePrecision) : "",
      sell_decimal_digits:
        amountPrecision !== undefined ? String(amountPrecision) : "",
      max_buy_amount: maxAmount !== undefined ? String(maxAmount) : "",
      max_sell_amount: maxAmount !== undefined ? String(maxAmount) : "",
      custom_fee_rate: undefined,
      enable: true,
    };

    await toast.promise(addSpotTradingPair(pair, token), {
  ...
  async function addFromMarket(market: any) {
    ...
    try {
      ...
      const chainIds = Array.from(
        new Set([...getChainIds(baseAssets), ...getChainIds(quoteAssets)]),
      );
      await Promise.all(chainIds.map((chainId) => ensureChainInfo(chainId)));

      if (hasMultipleChains(baseAssets) || hasMultipleChains(quoteAssets)) {
  ...
  async function confirmAssetSelection() {
    ...
    try {
      await submitPair(pendingMarket, selectedBaseAsset, selectedQuoteAsset);
      resetAssetPicker();
    } finally {
      addingKey = null;
    }
        ...
        {#if showAssetPicker}
          <div class="rounded-lg border border-base-200 bg-base-200/40 p-4 space-y-4">
            <div class="flex items-center justify-between">
              <div>
                <div class="font-semibold">{$_("select_assets")}</div>
                <div class="text-xs text-base-content/60">{$_("select_assets_hint")}</div>
              </div>
              <button class="btn btn-xs btn-ghost" on:click={resetAssetPicker}>
                {$_("cancel")}
              </button>
            </div>

            <div class="space-y-2">
              <div class="text-xs font-semibold capitalize tracking-wide opacity-60">
                {$_("base_asset_choice", { values: { symbol: pendingMarket?.base } })}
              </div>
              {#each baseAssetOptions as asset}
                <label class="flex items-center gap-3 p-3 rounded-lg border border-base-200 bg-base-100">
                  <input
                    type="radio"
                    name="base-asset"
                    class="radio radio-sm"
                    checked={selectedBaseAsset?.asset_id === asset.asset_id}
                    on:change={() => (selectedBaseAsset = asset)}
                  />
                  <img src={asset.icon_url} alt={asset.symbol} class="w-8 h-8 rounded-full" />
                  <div class="flex-1">
                    <div class="font-semibold">{asset.symbol}</div>
                    <div class="text-xs text-base-content/60">{asset.name}</div>
                  </div>
                  <div class="flex items-center gap-2 text-xs">
                    {#if chainInfoById[asset.chain_id]?.icon_url}
                      <img
                        src={chainInfoById[asset.chain_id].icon_url}
                        alt={chainInfoById[asset.chain_id].symbol}
                        class="w-4 h-4 rounded-full"
                      />
                    {/if}
                    <span class="font-mono opacity-70">{asset.chain_id}</span>
                  </div>
                </label>
              {/each}
            </div>
            ...
            <div class="space-y-2">
              <div class="text-xs font-semibold capitalize tracking-wide opacity-60">
                {$_("quote_asset_choice", { values: { symbol: pendingMarket?.quote } })}
              </div>
              {#each quoteAssetOptions as asset}
                <label class="flex items-center gap-3 p-3 rounded-lg border border-base-200 bg-base-100">
                  <input
                    ...
                    name="quote-asset"
                    class="radio radio-sm"
                    checked={selectedQuoteAsset?.asset_id === asset.asset_id}
                    on:change={() => (selectedQuoteAsset = asset)}
                  />
                  <img src={asset.icon_url} alt={asset.symbol} class="w-8 h-8 rounded-full" />
                  <div class="flex-1">
                    <div class="font-semibold">{asset.symbol}</div>
            ...
            >
              ...
              {:else}
                {$_("add_pair_action", { values: { symbol: pendingMarket?.symbol } })}
              {/if}
            </button>
          </div>
        {/if}
              ...
              <div class="flex items-center justify-between gap-4 p-3 rounded-lg border border-base-200">
                ...
                <button
                  class={clsx(
                    ...
                    addingKey === `${market.exchange_id}:${market.symbol}` && "btn-disabled",
                  )}
