<script lang="ts">
  ...
  const fundAmountValue = Number(campaign.fund_amount) / Math.pow(10, campaign.fund_token_decimals);
  const balanceValue = Number(campaign.balance) / Math.pow(10, campaign.fund_token_decimals);
  const paidValue = fundAmountValue - balanceValue;
  const progressPercent = fundAmountValue > 0 ? (paidValue / fundAmountValue) * 100 : 0;
...
<div class="flex flex-col space-y-6 p-6 md:p-8 pb-6">
  ...
  <div class="flex flex-col space-y-4">
    ...
    <div class="flex flex-wrap items-center gap-2">
      ...
      >
      <span class="badge {getStatusBadgeClass(campaign.status)} font-medium"
        >{formatStatus(campaign.status)}</span
      >
      <span class="badge {getEscrowBadgeClass(campaign.escrow_status)} font-medium"
        >Escrow: {campaign.escrow_status}</span
      >
    </div>
  </div>
  ...
  <div class="grid grid-cols-2 gap-3">
    <div class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-4">
      ...
      <div class="text-lg font-bold">{fundAmountValue.toLocaleString()} {campaign.fund_token_symbol}</div>
    </div>
    <div class="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-4">
      <div class="text-xs text-blue-600 font-medium mb-1">Current Balance</div>
      <div class="text-lg font-bold text-blue-700">{balanceValue.toLocaleString()} {campaign.fund_token_symbol}</div>
    </div>
    <div class="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-4">
      <div class="text-xs text-green-600 font-medium mb-1">Amount Paid</div>
      <div class="text-lg font-bold text-green-700">{paidValue.toLocaleString()} {campaign.fund_token_symbol}</div>
    </div>
    <div class="bg-gradient-to-br from-amber-50 to-amber-100 rounded-xl p-4">
      <div class="text-xs text-amber-600 font-medium mb-1">{getTargetLabel(campaign.type)}</div>
      <div class="text-lg font-bold text-amber-700">{getTargetValue(campaign)}</div>
    </div>
  </div>
  ...
  <!-- Oracle Fees -->
  <div class="flex flex-col space-y-4">
    <div class="font-bold text-sm bg-slate-50 p-3 px-4 rounded-lg">Oracle Fees</div>
    <div class="flex flex-col space-y-4 px-2">
      <div class="flex justify-between">
      ...
      <div class="flex justify-between items-center">
        ...
        <div class="flex items-center gap-2">
          ...
          {#if getCampaignResults(campaign) !== "N/A"}
            <a
              href={campaign.final_results_url || campaign.intermediate_results_url || ''}
              target="_blank"
            ...
            {#each campaign.daily_paid_amounts as payment}
              <div class="flex justify-between text-sm">
                ...
                <span class="font-medium"
                  >{payment.amount} {campaign.fund_token_symbol}</span
                >
              </div>
            {/each}
          </div>
