...
<div class="flex flex-col rounded-xl border border-base-200 relative shadow-sm">
  ...
  >
    ...
    <div class="flex justify-between items-center w-full">
      <div class="flex space-x-2 justify-start">
        ...
        <div class="flex avatar items-center">
          <div class="w-6 h-6 rounded-full">
            <img
              class=""
              src={findExchangeIconByIdentifier(data.exchangeName)}
              alt=""
            />
          </div>
        </div>
      ...
      <span
        class="text-xs text-green-600 border-green-600 px-2.5 bg-base-100 border rounded-md text-nowrap"
      >
        {$_("market_making")}
      </span>
    </div>
      ...
      <div class="flex flex-col space-y-2 text-xs justify-start">
        <div class="flex space-x-2 items-center mt-1">
          ...
          <span class="uppercase text-[10px] font-medium"
            >{data.pair.replaceAll("-ERC-20", "")}</span
          >
        </div>
        <div class="flex space-x-2 items-center mt-1">
          <!-- clock icon -->
          ...
          <span class="text-[10px] font-medium">
            {Number(daysBetweenToday(data.createdAt)) > 1
              ? $_("created_days_ago", {
                  values: { days: daysBetweenToday(data.createdAt) },
                })
              : $_("recently_created")}
          </span>
        </div>
