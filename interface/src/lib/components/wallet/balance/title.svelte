...
<div class="flex flex-col">
    ...
    <div class="relative flex justify-center my-8 mb-3 space-x-2 mx-auto">
        ...
        {#if $userAssets.totalUSDBalance}
            <span class="text-4xl font-medium balance-font">
                {$userAssets.totalUSDBalance <= 1
                    ? formatDecimals($userAssets.totalUSDBalance, 4)
                    : formatDecimals($userAssets.totalUSDBalance, 2)}
            </span>
        {:else}
            <span class="text-4xl font-medium balance-font"> 0 </span>
        {/if}
    </div>
    ...
    <div class="flex justify-center items-center space-x-2">
        ...
        {:else}
            <span class="balance-font"> 0 </span>
        {/if}
        <span class="text-xs opacity-50"> BTC </span>
    </div>
</div>
