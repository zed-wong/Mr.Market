...
{#if show}
  <div class="modal modal-open bg-black/20 backdrop-blur-[2px]">
    <div class="modal-box bg-base-100 p-0 rounded-2xl max-w-[480px] shadow-2xl border border-base-200/50 max-h-[90vh] overflow-y-auto">
      <!-- Header -->
      <div class="px-7 pt-6 pb-5">
        ...
        <span class="text-sm text-base-content/50">{$_("admin_direct_mm_configure_deploy")}</span>
      </div>
      ...
      <div class="px-7 pb-7 flex flex-col gap-5">
        ...
        <div class="bg-slate-50/50 rounded-xl p-4">
          ...
          <select
            class="select select-bordered w-full h-10 min-h-[40px] bg-base-100 text-base-content focus:outline-none focus:border-primary border-base-200"
        ...
        <div class="bg-slate-50/50 rounded-xl p-4">
          ...
          <div class="relative">
            ...
            <input
              class="input input-bordered w-full h-10 min-h-[40px] pl-9 bg-base-100 text-base-content text-sm focus:outline-none focus:border-primary border-base-200"
              placeholder={$_("admin_direct_mm_search_pairs")}
              bind:value={pairSearch}
            />
          </div>
          {#if pairSearch && searchedPairs.length > 0}
            <div class="mt-2 max-h-32 overflow-y-auto border border-base-200 rounded-lg bg-base-100 shadow-sm">
        ...
        <div>
          ...
          <div class="flex gap-3">
            <div class="flex-1 bg-slate-50/50 rounded-xl p-4">
              <span class="text-xs font-semibold text-base-content/50 tracking-wider block mb-2">{$_("admin_direct_mm_base_amount")}</span>
              <input
                type="text"
                placeholder="0.00"
                class="input input-bordered w-full h-10 min-h-[40px] bg-base-100 text-base-content text-sm focus:outline-none focus:border-primary border-base-200"
