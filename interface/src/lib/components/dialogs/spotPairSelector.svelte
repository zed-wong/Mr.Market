<script lang="ts">
  ...
  import { getSpotTradingPairs } from "$lib/helpers/mrm/coin";
  import { switchSpotPair } from "$lib/helpers/mrm/socket";
  ...
  $: filteredPairs = pairs
    ? pairs
        .filter((item) => {
          return $pairExchangeFilter.toUpperCase() === "ALL"
            ? item
            : item.exchange
                .toUpperCase()
                .match($pairExchangeFilter.toUpperCase());
        })
        .filter((item) => {
          return (
...
>
  <div
    ...
    data-testid="spot_pair_selector_modal_box"
  >
      ...
      <div class="absolute left-[calc(50%-16px)] top-2">
        <div class="w-8 h-1 bg-slate-800/40 rounded-full">
          <form method="dialog" class="modal-backdrop">
            <button
              on:click={() => pairSelectorDialog.set(false)}
              data-testid="spot_pair_selector_close">c</button
            >
          </form>
        </div>
      </div>
      ...
      <div class="tabs border-b w-full mt-3 px-3 overflow-x-auto no-scrollbar">
        {#each tabItems as item}
          <button
            class={clsx(
              "tab",
              $pairExchangeFilter === item.name &&
                "border-b border-base-content",
            )}
            on:click={() => {
              pairExchangeFilter.set(item.name);
            }}
          >
            <span
              class={clsx(
      ...
      <div class="flex flex-col overflow-y-auto mt-0 px-4">
        ...
        {:else}
          {#each filteredPairs as c}
            <div
              class="w-full flex items-center justify-start space-x-2 py-3 h-16"
            >
              <button
                class="flex justify-between w-full items-center"
                data-dismiss="select_pair_modal"
                on:click={() => {
              ...
              >
                <div class="flex items-center space-x-2.5">
                  <img
                    src={findExchangeIconByIdentifier(c.exchange)}
                    alt="-"
                    loading="lazy"
                    class="w-5 h-5"
                  />
                  <span class="flex items-center font-semibold text-sm">
                    {c.symbol.split("/")[0]}<span
                      class="font-light text-xs text-base-content/60"
                      >/{c.symbol.split("/")[1]}</span
                    >
                  </span>
                </div>
                ...
                <div class="flex flex-col items-end">
                  ...
                  {#if c.change}
                    <span
                      class={clsx(
                        "text-xs !text-[10px]",
                        c.change >= 0 ? UpColorText : DownColorText,
                      )}
                    >
